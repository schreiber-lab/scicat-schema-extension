...
const fetchMetadaSchemas =
  (params) =>
  ({ loadedOptions = [] }) => {
    return metadataSchemasApi
      .getMetadataSchemas({
        params: {
          //   page: page + 1,
      ...
      .then((data) => {
        return {
          ...
          options: loadedOptions.concat(
            data.filter(({ fixed_value_entries }) => fixed_value_entries)
          ),
...
export const MetadataSchemasAutocomplete = ({
  params = {},
  creatablePayload,
  ...props
}) => {
  return (
    <Autocomplete
      ...
      getOptionSelected={(option, value) =>
        option?.schema_name === value?.schema_name
      }
