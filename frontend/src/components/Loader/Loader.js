...
export const Loader = ({
  ...
  const setSpinnerSize = () => {
    if (loading) {
      ...
      const spinnerSize =
        smallerSide < DEFAULT_SPINNER_SIZE && surface
          ? smallerSide
          : DEFAULT_SPINNER_SIZE;

      setSpinnerSizeState(
        spinnerSize + (surface ? spacing * (inset ? -0.5 : 1) : 0)
      );
    }
  };
  ...
  return loading ? (
    <Box
      ...
      className={cn(
        classes.root,
        { [classes.root_surface]: surface },
        className
      )}
      {...props}
    >
      <div className={classes.content}>{inset && surface && render()}</div>
